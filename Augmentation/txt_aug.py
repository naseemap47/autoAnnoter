...
ap.add_argument("-t", "--txt", type=str, required=True,
                help="path to (YOLO) txt/dir")
ap.add_argument("-s", "--save", type=str, required=True,
                help="path to save aug yolo (txt)")
...
generate_anotT_RandomHSV(hsv_h_img, args['txt'], hue=data['hsv_h']['hue'], path_to_save=args['save'], name_id='hsv_h')
...
generate_anotT_RandomHSV(hsv_s_img, args['txt'], saturation=data['hsv_s']['saturation'], path_to_save=args['save'], name_id='hsv_s')
...
generate_anotT_RandomHSV(hsv_v_img, args['txt'], brightness=data['hsv_v']['brightness'], path_to_save=args['save'], name_id='hsv_v')
...
generate_anotT_RandomHSV(
    hsv_img, args['txt'], hue=data['hsv']['hue'], saturation=data['hsv']['saturation'], 
...
generate_anotT_Rotate(rot_img, args['txt'], args['save'], 'rot', data['degrees']['deg'])

# image Random rotation augmentation
total_prob = int((data['degrees_random']['prob'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_RandomRotate(rot_img, args['txt'], args['save'], 'rot_random', data['degrees_random']['deg'])

# image Translate augmentation
total_prob = int((data['translate']['prob'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_Translate(rot_img, args['txt'], args['save'], 'trans', data['translate']['translate_x'], data['translate']['translate_y'])

# image Random Translate augmentation
total_prob = int((data['translate_random']['prob'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_RandomTranslate(rot_img, args['txt'], args['save'], 'trans_random', data['translate_random']['translate'])

# image Scale augmentation
total_prob = int((data['scale']['prob'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_Scale(rot_img, args['txt'], args['save'], 'scale', data['scale']['scale_x'], data['scale']['scale_y'])

# image Random Scale augmentation
total_prob = int((data['scale_random']['prob'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_RandomScale(rot_img, args['txt'], args['save'], 'scale_random', data['scale_random']['scale'])

# image Shear augmentation
total_prob = int((data['shear']['prob'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_Shear(rot_img, args['txt'], args['save'], 'shear', data['shear']['shear'])

# image Random Shear augmentation
total_prob = int((data['shear_random']['prob'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_RandomShear(rot_img, args['txt'], args['save'], 'shear_random', data['shear_random']['shear'])

# image flip up-down augmentation
total_prob = int((data['flipud'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_flipud(rot_img, args['txt'], args['save'], 'flipud')

# image flip left-right augmentation
total_prob = int((data['fliplr'])*len(img_list))
rot_img = choice(img_list, total_prob, replace=False)
generate_anotT_fliplr(rot_img, args['txt'], args['save'], 'fliplr')
